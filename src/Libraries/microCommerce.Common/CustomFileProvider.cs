...
namespace microCommerce.Common
{
    public class CustomFileProvider : PhysicalFileProvider, ICustomFileProvider
    {
        ...
        public CustomFileProvider(IHostingEnvironment hostingEnvironment)
             : base(File.Exists(hostingEnvironment.WebRootPath) ?
                   Path.GetDirectoryName(hostingEnvironment.WebRootPath) :
                   hostingEnvironment.WebRootPath)
        {
            _webRootPath = hostingEnvironment.ContentRootPath;
            _contentRootPath = hostingEnvironment.WebRootPath;
        }
        ...
        /// </summary>
        /// <param name="path">The path to map. E.g. "~/images"</param>
        /// <returns>The physical path. E.g. "c:\inetpub\wwwroot\images"</returns>
        public virtual string MapPath(string path)
        {
            path = path.Replace("~/", string.Empty).TrimStart('/').Replace('/', '\\');
            return Path.Combine(_webRootPath ?? string.Empty, path);
        }
        ...
        /// </summary>
        /// <param name="path">The path to map. E.g. "~/images"</param>
        /// <returns>The physical path. E.g. "c:\inetpub\images"</returns>
        public virtual string MapContentPath(string path)
        {
            path = path.Replace("~/", "").TrimStart('/').Replace('/', '\\');
            return Path.Combine(_contentRootPath, path);
        }
